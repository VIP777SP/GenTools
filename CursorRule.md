...
## ディレクトリ構造について
- プロジェクトは `genshin-tools/genshin-tools` という二重構造
- 間違って外側のディレクトリにファイルを作成しないよう注意
- 正しいプロジェクトルートは内側の `genshin-tools` ディレクトリ
- 開発サーバーは正しいディレクトリで起動済み

## トップページ作成 (2024-12-XX)
- **美しいランディングページを実装**
  - ヒーローセクション: 「Genshin Tools」タイトル、アニメーションアイコン、CTAボタン
  - 機能紹介セクション: 3つのカード（ツール集、攻略記事、リンク集）
  - 統計セクション: 97+キャラクター、104+武器、51+聖遺物、4+便利ツール
  - フッターCTA: 最終行動喚起
- **デザイン特徴**:
  - グラデーション背景（slate-blue-indigo）
  - ガラスモーフィズムカード
  - React Icons使用（IoStarOutline等）
  - ホバーアニメーション、レスポンシブ対応
- **ナビゲーション**: /gs-tools, /gs-articles, /gs-links へのリンク

## z-index階層調整 (2024-12-XX)
- **適切な重なり順序を実装**
  - サイドバー: z-[100] (最上位)
  - サイドバーオーバーレイ: z-[90]
  - Tierメーカーラベル設定パネル: z-[85]
  - Tierメーカーラベル設定オーバーレイ: z-[80]
  - ビルド設定モーダル: z-[75]
  - Tierメーカーキャラリスト: z-[70]
  - フッター: z-40
- **UX改善**: サイドバー > ラベル設定 > キャラリストの正しい階層
